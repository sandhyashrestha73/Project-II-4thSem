...
export default function Destinations() {
  ...
  const filtered = destinations.filter((d) => {
    ...
    return d.name.toLowerCase().includes(q) || d.district.toLowerCase().includes(q);
  });
    ...
    <div>
      <section className="bg-base-surface py-16">
        <div className="mx-auto max-w-7xl px-4 md:px-8">
          ...
          <p className="mt-2 text-text-muted">Explore places across Nepal covered by our agencies.</p>
          <input
            value={query}
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 md:px-8">
        ...
        {!loading && !error && filtered.length === 0 && (
          <p className="text-text-muted">No destinations found.</p>
        )}
        {!loading && !error && filtered.length > 0 && (
