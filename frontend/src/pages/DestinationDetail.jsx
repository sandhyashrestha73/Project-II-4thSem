  ...
  function load() {
    ...
    Promise.all([getDestination(id), getPackages()])
      .then(([dest, allPackages]) => {
        setDestination(dest);
        setPackages(allPackages.filter((p) => String(p.destination_id) === String(id)));
      })
      .catch((err) => setError(extractErrorMessage(err, "Destination not found.")))
  ...
  return (
    <div>
      ...
      <div className="mx-auto max-w-5xl px-4 py-10 md:px-8">
        <h1 className="text-3xl font-extrabold text-text-main md:text-4xl">{destination.name}</h1>
        <p className="mt-1 text-accent-secondary">{destination.district}</p>
        <p className="mt-6 leading-relaxed text-text-muted">
          {destination.description || "No description provided for this destination yet."}
        </p>

        <h2 className="mt-12 text-2xl font-bold text-text-main">Packages for this destination</h2>
        {packages.length === 0 ? (
          <p className="mt-4 text-text-muted">No packages currently cover this destination.</p>
        ) : (
        ...
        <Link to="/destinations" className="mt-10 inline-block text-sm font-medium text-accent-secondary hover:underline">
