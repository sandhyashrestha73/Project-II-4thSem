...
export default function AgencyPackages() {
  ...
  function load() {
    ...
    Promise.all([getPackages(), getDestinations()])
      .then(([pkgs, dests]) => {
        setPackages(pkgs.filter((p) => String(p.agency_id) === String(user.id)));
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setFormError(extractErrorMessage(err, "Could not save this package."));
    } finally {
      setSaving(false);
    }
  }
      ...
      <div className="mt-8">
        ...
        {!loading && !error && packages.length > 0 && (
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {packages.map((p) => (
              <div key={p.package_id} className="card overflow-hidden">
                <div className="h-36 w-full bg-base-surface">
                  {p.image ? (
                    <img src={p.image} alt={p.package_name} className="h-full w-full object-cover" />
                  ) : (
                    <div className="flex h-full items-center justify-center text-slate-600">No image</div>
                  )}
                </div>
                <div className="p-4">
                  <p className="font-semibold text-white">{p.package_name}</p>
                  <p className="mt-1 text-sm text-slate-400">{p.duration}</p>
                  <p className="mt-2 font-bold text-accent">NPR {Number(p.price).toLocaleString()}</p>
                  <div className="mt-4 flex gap-2">
                    <button onClick={() => openEdit(p)} className="btn-secondary flex-1 text-sm">
                      Edit
                    </button>
                    <button onClick={() => handleDelete(p.package_id)} className="btn-danger flex-1 text-sm">
                      Delete
                    </button>
                  </div>
                </div>
